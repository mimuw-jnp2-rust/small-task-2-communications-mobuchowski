...
impl Client {
    ...
    fn open(&mut self, addr: &str, server: Server) -> CommsResult<()> {
        match self.connections.get(addr) {
            ...
            Some(Connection::Open(_)) => Err(CommsError::ConnectionExists(addr.to_string())),
            None => {
                self.connections
    ...
    fn send(&mut self, addr: &str, msg: Message) -> CommsResult<Response> {
        match self.connections.get_mut(addr) {
            ...
            Some(Connection::Closed) => Err(CommsError::ConnectionClosed(addr.to_string())),
            Some(Connection::Open(srv)) => match srv.receive(msg) {
                Err(err) => {
                    if let CommsError::ServerLimitReached(str) = err {
                        self.connections
                            .insert(addr.to_string(), Connection::Closed);
                        Err(CommsError::ServerLimitReached(str))
                    } else {
                        Err(err)
                    }
                }
                Ok(x) => CommsResult::Ok(x),
            },
        }
    }
    ...
    fn is_open(&self, addr: &str) -> bool {
        !matches!(self.connections.get(addr), None | Some(Connection::Closed))
    }
